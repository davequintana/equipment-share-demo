  ...
  return new Promise((resolve) => {
    ...
    const request = protocol.get(service.url, (res) => {
      if (res.statusCode >= 200 && res.statusCode < 400) {
        ...
        log(
          colors.yellow,
          `⚠️  ${service.name} - Unexpected status (${res.statusCode})`,
        );
        resolve(false);
      }
    });
...
const verifyStartup = async () => {
  log(
    colors.blue,
    `${colors.bold}🔍 Verifying startup services...${colors.reset}`,
  );
  console.log('');
  ...
  if (allRunning) {
    log(
      colors.green,
      `${colors.bold}🎉 All services are running successfully!${colors.reset}`,
    );
    ...
    log(
      colors.reset,
      '   2. Open http://localhost:8080 to monitor Kafka events',
    );
    log(
      colors.reset,
      '   3. User interactions will be tracked and sent to Kafka',
    );
    return true;
  } else {
    log(
      colors.yellow,
      `${colors.bold}⚠️  Some services may still be starting up${colors.reset}`,
    );
    ...
    log(
      colors.reset,
      '   • Restart if needed: pnpm run docker:down && pnpm run start:full',
    );
    return false;
  }
};
