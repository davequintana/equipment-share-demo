  ...
  webServer: [
    ...
    },
    {
      command: process.env.CI
        ? 'bash -c "echo \\"Checking fastify-api deployment...\\" && if [ ! -d dist/apps/fastify-api ]; then echo \\"Building fastify-api locally...\\" && pnpm exec nx build fastify-api --prod && pnpm exec nx build secrets && echo \\"Checking build outputs:\\" && find . -name \\"secrets\\" -type d 2>/dev/null || echo \\"No secrets directories found\\" && echo \\"Setting up workspace dependencies...\\" && mkdir -p dist/apps/fastify-api/node_modules && if [ -d libs/secrets/dist ]; then echo \\"Copying from libs/secrets/dist\\" && cp -r libs/secrets/dist dist/apps/fastify-api/node_modules/secrets; elif [ -d dist/libs/secrets ]; then echo \\"Copying from dist/libs/secrets\\" && cp -r dist/libs/secrets dist/apps/fastify-api/node_modules/secrets; else echo \\"⚠️  Secrets package not found - fastify-api will run without workspace dependencies\\"; fi && echo \\"Creating package.json for npm start...\\" && echo \'{\\"name\\": \\"fastify-api\\", \\"version\\": \\"1.0.0\\", \\"main\\": \\"main.js\\", \\"scripts\\": {\\"start\\": \\"node main.js\\"}}\' > dist/apps/fastify-api/package.json && echo \\"✅ Workspace setup complete\\"; fi && cd dist/apps/fastify-api && npm start"'
