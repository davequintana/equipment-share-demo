  ...
  webServer: [
    ...
    },
    {
      command: process.env.CI
        ? 'bash -c "echo \\"Using existing fastify-api build from CI...\\" && if [ -d dist/apps/fastify-api ]; then echo \\"✅ Found existing build\\" && echo \\"Setting up workspace dependencies...\\" && mkdir -p dist/apps/fastify-api/node_modules && if [ -d libs/secrets/dist ]; then echo \\"Copying from libs/secrets/dist\\" && cp -r libs/secrets/dist dist/apps/fastify-api/node_modules/secrets; elif [ -d dist/libs/secrets ]; then echo \\"Copying from dist/libs/secrets\\" && cp -r dist/libs/secrets dist/apps/fastify-api/node_modules/secrets; else echo \\"⚠️  Secrets package not found - fastify-api will run without workspace dependencies\\"; fi && echo \\"Creating package.json for npm start...\\" && echo \'{\\"name\\": \\"fastify-api\\", \\"version\\": \\"1.0.0\\", \\"main\\": \\"main.js\\", \\"scripts\\": {\\"start\\": \\"node main.js\\"}}\' > dist/apps/fastify-api/package.json && echo \\"✅ Workspace setup complete\\"; else echo \\"❌ No existing build found - check artifact download\\" && exit 1; fi && cd dist/apps/fastify-api && npm start"'
